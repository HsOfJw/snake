...
cc._RF.push(module, '4fb61pveAdGiqF/LaUyVw1a', 'GameScene', __filename);
...
cc.Class({
    ...
    joinNextLevel: function joinNextLevel() {
    ...
    //初始化地图信息
    initMap: function initMap(level_id) {
        var self = this;
    ...
    //显示即将超越的玩家
    showJJCYView: function showJJCYView() {
        var self = this;
    ...
    initEventListen: function initEventListen() {
        var self = this;
        ...
        Game.EventCenter.RegisterEvent(this.uuid, Game.MessageType.Snake_Drop_Finish, function () {
            if (Game.GlobalVar.isGameOver) return;
    ...
    //生成蛇
    generateSnake: function generateSnake() {
        //创建蛇
        this.snakes.create();
    ...
    //生成门
    generateDoor: function generateDoor() {
    ...
    recreateTiled: function recreateTiled() {
        if (!this.shelterTileVec) {
            this.shelterTileVec = [];
        }
    ...
    //刷新石头的GID
    updateStoneGID: function updateStoneGID() {
        Game.MapUtil.UpdateStoneGID(this.stoneMgr.GetAllStoneGID());
    },


    //刷新蛇的GID
    updateSnakeGID: function updateSnakeGID() {
    ...
    move: function move(moveDirection) {
        if (this.isMoving) {
            return;
        }
    ...
    //判断蛇头是否与苹果同边相邻
    isAdjacentToApple: function isAdjacentToApple() {
    ...
    //判断蛇头是否与地刺同边相邻
    isAdjacentToDici: function isAdjacentToDici() {
    ...
    isEatingApple: function isEatingApple(moveDirection) {
    ...
    //判断是否被刺死了
    isDeatch: function isDeatch(direction) {
        if (Game.GlobalVar.isDroping) return false;
    ...
    //是否碰到了地刺
    isTouchDiCi: function isTouchDiCi(direction) {
        var diciGID = Game.MapUtil.GetDiCiGID();
        var curGID = this.snakes.getHeadCurGID();
    ...
    //判断下一个操作是否胜利
    isWin: function isWin(direction) {
    ...
    //刷新最短时间
    refreshMinTimeUI: function refreshMinTimeUI(time) {
    ...
    //生成过关遮罩
    outLevelMask: function outLevelMask(direction) {
    ...
    //选择关卡按钮点击
    selectLevelClicked: function selectLevelClicked() {
        if (this.isStart == false) return;
    ...
    //重玩按钮点击
    replayBtnClicked: function replayBtnClicked() {
        if (this.isStart == false) return;
    ...
    //提示重玩动画
    replayEffect: function replayEffect() {
        // this.replayBtn.runAction(new cc.repeatForever(new cc.blink(0.3, 1)));
    },


    //取消提示重玩
    closeReplayEffect: function closeReplayEffect() {
        if (this.replayBtn) {
    ...
    //游戏结束
    gameOver: function gameOver(isWin) {
